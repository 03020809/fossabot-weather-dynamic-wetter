...
app.get('/weather', async (req, res) => {
  try {
    ...
    let lon = req.query.lon || DEFAULT_LON;
    let name = req.query.city || LOCATION_NAME;

    // Wenn cityname übergeben wurde, Koordinaten per Geocoding holen
    if (req.query.cityname) {
      const geo = await geocodeCity(fetch, req.query.cityname);
      if (!geo) {
        return res.send(`Stadt "${req.query.cityname}" nicht gefunden.`);
      }
      lat = geo.lat;
      lon = geo.lon;
      name = `🌍 ${geo.name} `;
